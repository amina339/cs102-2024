    ...
    def run(self):
        ...
        while True:
            ...
            if not self.life.is_max_generations_exceeded and self.life.is_changing:
                ...
                screen.clear()
                screen.addstr(0, 0, "The number of possible generations has exceeded" '\t Please, press "X" to exit')
                screen.refresh()
                sign = False
            elif not self.life.is_max_generations_exceeded and not self.life.is_changing:
                screen.clear()
                screen.addstr(0, 0, "You have reached the final state of life " '\t Please, press "X" to exit')
                screen.refresh()
                sign = False
            time.sleep(0.5)
            key = screen.getch()
...
if __name__ == "__main__":
    game = GameOfLife((30, 30), max_generations=10)
    ui = Console(game)
    ui.run()
